        ...
        new(nameof(ClashSubTemplate), ClashSubTemplate, new()
        {
            ...
            [
                new("custom",
                    ...
                    new("custom_proxy_group", "节点选择`select`[]自动选择`[]DIRECT`.*"),
                    new("custom_proxy_group", "自动选择`url-test`.*`http://www.gstatic.com/generate_204`300,,50"),
                    new("custom_proxy_group", "全球直连`select`[]DIRECT`[]节点选择`[]自动选择"),
                    new("custom_proxy_group", "全球拦截`select`[]REJECT`[]DIRECT"),
                    new("custom_proxy_group", "漏网之鱼`select`[]节点选择`[]全球直连`[]自动选择`.*"),
